 ...
 *
 * Idempotency guard: when `attrs['data-trackgate-id']` is present, checks the DOM for
 * an existing `script[data-trackgate-id="<id>"]` and no-ops if found. This is the
...
export function loadConsentedScript(
  ...
  const trackgateId = attrs?.['data-trackgate-id'];
  if (trackgateId) {
    const existing = document.querySelector<HTMLScriptElement>(
      `script[data-trackgate-id="${trackgateId}"]`,
    );
